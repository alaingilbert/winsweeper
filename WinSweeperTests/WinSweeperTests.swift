...
class CreateGroupsTests: XCTestCase {
    func test1() {
        let gb = GameBoard(width: 19, height: 13)
        let tiles = [Tile(ctx: gb, idx: 0), Tile(ctx: gb, idx: 1), Tile(ctx: gb, idx: 2)]
        let groups = gb.createGroups(tiles)
        XCTAssertEqual(groups.count, 1)
        XCTAssertEqual(groups[0][0].idx(), 0)
        XCTAssertEqual(groups[0][1].idx(), 1)
        XCTAssertEqual(groups[0][2].idx(), 2)
    }
    
    func test2() {
        let gb = GameBoard(width: 19, height: 13)
        let tiles = [Tile(ctx: gb, idx: 0), Tile(ctx: gb, idx: 1), Tile(ctx: gb, idx: 3), Tile(ctx: gb, idx: 4), Tile(ctx: gb, idx: 5)]
        let groups = gb.createGroups(tiles)
        XCTAssertEqual(groups.count, 1)
        XCTAssertEqual(groups[0][0].idx(), 0)
        XCTAssertEqual(groups[0][1].idx(), 1)
        XCTAssertEqual(groups[0][2].idx(), 3)
        XCTAssertEqual(groups[0][3].idx(), 4)
        XCTAssertEqual(groups[0][4].idx(), 5)
    }
    
    func test3() {
        let gb = GameBoard(width: 19, height: 13)
        let tiles = [Tile(ctx: gb, idx: 0), Tile(ctx: gb, idx: 1), Tile(ctx: gb, idx: 4), Tile(ctx: gb, idx: 5), Tile(ctx: gb, idx: 6)]
        let groups = gb.createGroups(tiles)
        XCTAssertEqual(groups.count, 2)
        XCTAssertEqual(groups[0][0].idx(), 0)
        XCTAssertEqual(groups[0][1].idx(), 1)
        XCTAssertEqual(groups[1][0].idx(), 4)
        XCTAssertEqual(groups[1][1].idx(), 5)
        XCTAssertEqual(groups[1][2].idx(), 6)
    }
}
